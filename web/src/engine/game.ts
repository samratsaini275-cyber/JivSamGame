...
export class Game {
  ...
  /// Dev toggle (mirrors the Swift app's Persistence.enabled): while false,
  /// the app neither loads nor writes the localStorage save — every refresh
  /// is a fresh run. Tests call loadState/saveState directly and are unaffected.
  static persistenceEnabled = false;

  static loadOrNew(): Game {
    if (!Game.persistenceEnabled) return new Game(newGame());
  ...
  private save(): void {
    if (!Game.persistenceEnabled) return;
